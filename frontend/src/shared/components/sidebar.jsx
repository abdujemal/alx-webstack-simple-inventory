            ...
            <div className="flex flex-col pt-10 pr-8 mx-auto w-full text-lg text-white pb-[636px] shadow-[0px_4px_4px_rgba(0,0,0,0.25)] max-md:pb-24 max-md:mt-2.5">
                ...
                <menu className='mt-20'>
                    ...
                    menus.map((e)=> 
                        e.path != '/logout'  ?
                        <Link to={e.path}>
                            <div className={`cursor-pointer hover:underline flex gap-1 ${ pathname === e.path ? "py-3.5" : "" } pr-4 pl-3 mt-7 w-full whitespace-nowrap ${ pathname === e.path ? "bg-white" : "" } rounded-e-full ${ pathname === e.path ? "text-zinc-800" : "text-white" } max-md:mt-10`}>
                                {e.icon}
                                <div>{e.name}</div>
